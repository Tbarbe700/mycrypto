...
export default function EquippedClaimBar({ claimAction, lastClaimedAtIso, items }: Props) {
  ...
  // si un claim se fait ailleurs => reset aussi
  useEffect(() => {
    const handler = (e: any) => {
      const nextIso = e?.detail?.lastClaimedAt as string | undefined
      anchorMsRef.current = nextIso ? safeDateMs(nextIso) : Date.now()

      // si c’est un claim équipés/inventaire, on reset le total “équipés” local
      // (au prochain tick, le total repartira de 0)
  ...
  async function claimEquipped() {
    ...
    try {
      ...
      // broadcast global (FarmCard, ItemPendingLive, etc.)

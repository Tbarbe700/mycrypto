...
export default function UpgradeButton({
  ...
  const doUpgrade = async () => {
    ...
    try {
      ...
      // 1) prepare
      const prepRes = await fetch("/api/items/upgrade/prepare", {
        method: "POST",
      ...
      // 3) finalize
      const finRes = await fetch("/api/items/upgrade/finalize", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const msg = e?.message || "Erreur upgrade"
      if (msg.includes("403")) {
  ...
  return (
    <div className="w-full">
      <button
        ...
        className="w-full rounded-lg border px-3 py-2 text-sm font-medium text-indigo-700 bg-white hover:bg-indigo-50 border-indigo-200 disabled:opacity-60"
      >
        {loading ? "Upgrade…" : label}
      </button>

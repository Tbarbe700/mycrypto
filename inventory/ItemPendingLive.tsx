...
export default function ItemPendingLive({
  ...
  // resync si navigation / server change
  useEffect(() => {
    setAnchorMs(anchorMsFromServer)
    setBasePending(base)
  }, [anchorMsFromServer, base])

  // écoute un claim global pour reset instant UI
  useEffect(() => {
    const handler = (e: any) => {
      ...
      setBasePending(0) // après claim, pending item repart de 0 (UI)
    }
  ...
  return (
    <div className="text-xs text-slate-500">
      En attente item : <span className="font-medium text-slate-900">{live.toFixed(2)}</span>
    </div>
  )
}

...
export default function SwapCard() {
  ...
  // ✅ Important : ne lit window que dans useEffect
  useEffect(() => {
    const p = getProvider()
    if (p?.isPhantom) {
  ...
  const connect = async () => {
    ...
    // Reset (corrige les états Phantom bizarres)
    try {
      await p.disconnect?.()
    } catch {}
  ...
  return (
    <div className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-baseline justify-between">
        <h2 className="text-sm font-semibold text-slate-900">Swap interne</h2>
        <div className="text-xs text-slate-500">1 SOL = {RATE} tokens</div>
      </div>
      ...
      <div className="mt-3">
        <label className="text-xs text-slate-600">Montant SOL</label>
        <input
          value={sol}

...
export async function POST(req: Request) {
  ...
  try {
    // 1) Freeze + calc claimable (inventory = not equipped)
    const preview = await prisma.$transaction(async (tx) => {
      await settlePending(tx, playerId)

      const items = await tx.playerItem.findMany({
        where: { playerId, equipped: false, pendingEarned: { gt: 0 } },
        select: { id: true, pendingEarned: true },
      })

      const claimedUi = items.reduce((sum, it) => sum + safeNum(it.pendingEarned, 0), 0)
      const ids = items.map((x) => x.id)

      return { claimedUi, ids }
    })
    ...
    // 3) Finalize DB: reset strict pendingEarned inventory + pendingBalance decrement + log
    await prisma.$transaction(async (db) => {
      ...
      // 🔥 RESET STRICT: ids preview (cibles) + on sécurise avec un fallback “equipped:false”
      if (preview.ids.length > 0) {
        await db.playerItem.updateMany({
          where: { id: { in: preview.ids } },
    ...
    console.error("CLAIM ALL INVENTORY ERROR:", e)
